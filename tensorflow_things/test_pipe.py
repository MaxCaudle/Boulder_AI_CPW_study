...
def idk_things_are_supposed_to_be_in_functions_i_guess_also_why_are_we_yelling(
    ...
    if type(test_images) == str:
        TEST_IMAGE_PATHS = get_images(test_images)
    else:
        ...
        with detection_graph.as_default():
            with tf.Session() as sess:
                for i, image_path in enumerate(TEST_IMAGE_PATHS):
                    ...
                    image_np = load_image_into_numpy_array(image)
                    image_np_expanded = np.expand_dims(image_np, axis=0)
                    dicts.append((image_path, run_inference_for_single_image(image, sess)))
...
if __name__ == '__main__':
    dicts, category_index = make_dicts(test_images='test_images',
         ...
         NUM_CLASSES=3, LABEL_MAP='object-detection.pbtxt', new_dict=True)
