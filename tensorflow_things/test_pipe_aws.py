...
def make_dicts(test_images,PATH_TO_CKPT, NUM_CLASSES, LABEL_MAP, new_dict = False, s3 = None):
    if new_dict:
            ...
            with tf.Session() as sess:
                for i, image in enumerate(test_images):
                    ...
                    try:
                        ...
                        image_np = load_image_into_numpy_array(image)
                        image_np_expanded = np.expand_dims(image_np, axis=0)
                        dicts.append((image_key.split('/')[1], run_inference_for_single_image(image, sess)))
                    except:
                        pass
                    if not i % 1000:
...
if __name__ == '__main__':
    ...
    dicts, category_index = make_dicts(test_images=objects,
         ...
         NUM_CLASSES=3, LABEL_MAP='object-detection.pbtxt', new_dict=True, s3=s3)
